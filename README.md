...
- 触屏视线跟随：眼睛先跟随，头部稍慢跟随；触摸不会写入身体纵向参数
- 参考 Soullink 的分层思路，加入非周期微动、随机注视、专注倾听和防重复低频自主动作
- 对模型声明范围和语义安全范围进行双重限幅，降低不同模型参数含义不一致导致的变形
- 轻透明文字测试面板：20 种动作、11 种情绪，以及 ZIP 自动发现的表情、外观和动作文件
